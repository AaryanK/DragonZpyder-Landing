...
async function submitViaClientAdapter(message, requestId) {
  ...
  try {
    ...
    // Operly PR #355 adds this retry-safe adapter. Until that stack reaches the
    // deployed backend, use the existing governed Personal Runtime endpoint.
...
async function sendMessage(message) {
  ...
  try {
    ...
    if (error instanceof ApiError && error.code === "execution_outcome_uncertain") {
      ...
      const meta = document.createElement("div");
      meta.className = "message-meta";
      meta.textContent = error.retryable
        ? `Safe retry identity: ${requestId}`
