...
function copyRequestHeaders(req, upstream) {
  ...
  // Operly's CSRF middleware intentionally requires the browser origin to match
  // the host that receives the authenticated request. DragonZpyder is a trusted
  // same-product reverse proxy, so normalize these server-to-server headers to
  // Operly while preserving the user's session/CSRF cookies and token header.
...
export default async function handler(req, res) {
  ...
  const init = {
    method,
    headers: copyRequestHeaders(req, upstream),
    redirect: "manual",
  };
